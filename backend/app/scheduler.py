...
# ─────────────────────────────────────────────────────────────────────────────
# EMAIL CONFIG — set these in your backend/.env (or Render env vars):
#
#   GMAIL_SENDER   = your.email@gmail.com
#   GMAIL_PASSWORD = your_16_char_app_password  ← NOT your real Gmail password!
#   FRONTEND_URL   = https://your-app.vercel.app  ← used in reminder email links
#
# How to get a Gmail App Password:
#   1. myaccount.google.com/security → Enable 2-Step Verification
#   2. Search "App passwords" → create one named "Applyt"
#   3. Copy the 16-character password → paste as GMAIL_PASSWORD
# ─────────────────────────────────────────────────────────────────────────────
...
def followup_checker():
    ...
    try:
        ...
        # Group by user_id so each user gets one consolidated email
        ...
        # Fetch user emails from Firebase Admin
        try:
            from firebase_admin import auth as firebase_auth
            for uid, followups in by_user.items():
                try:
                    user_record = firebase_auth.get_user(uid)
                    recipient   = user_record.email  # None for phone-only users
                except Exception:
                    recipient = None
...
def start_scheduler():
    ...
    # Run every 24 hours at 08:00 server time
    scheduler.add_job(followup_checker, "cron", hour=8, minute=0)
    # Also run once shortly after startup so you can verify it works
