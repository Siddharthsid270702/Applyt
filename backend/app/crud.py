...
def get_archived_applications(db: Session, user_id: str):
    return db.query(models.ArchivedApplication).filter(
        models.ArchivedApplication.user_id == user_id
    ).all()
...
def update_application(db: Session, app_id: int, update: schemas.ApplicationUpdate, user_id: str):
    ...
    if update.status == "Rejected":
        archive_application(db, app, "Rejected", user_id)
        return None
...
def restore_application(db: Session, archived_id: int, user_id: str):
    archived = db.query(models.ArchivedApplication).filter(
        models.ArchivedApplication.id == archived_id,
        models.ArchivedApplication.user_id == user_id
    ).first()
    ...
    app = models.Application(
        ...
        applied_date=archived.applied_date
    )
...
def mark_followup_sent(db: Session, followup_id: int, user_id: str):
    # Verify followup belongs to user via join
    followup = (
